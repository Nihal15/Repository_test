...
{
	/**
	 * Compares two {@link Node} structures. First criterion: node with lower in-degree is first; second criterion is lexical order.
	 * 
	 * @author Andrei Olaru
	 * 
	 */
	...
	public class PathElement
	{
		...
		int					treeOrder		= -1;								// order of the sub-tree containing this element
		...
		public String toString()
		{
			if(node == null)
				return "<corrupt>";
			return node.toString() + "(" + distance + ":" + (parent != null ? parent.node.toString() : "-") + (!children.isEmpty() ? "/" + children.size() : ".") + (otherChildren.isEmpty() ? "" : "+" + otherChildren.size()) + "/" + pathlength + ")";
		}
	}
	...
	protected void buildPaths()
	{
		...
		for(PathElement el : blackNodes)
		{
			...
			for(PathElement oth : el.otherChildren)
				...
				// 1) if the other child is already having the element as ancestor, then it is already on the main path and it is a forward link
				// 2) if the parent of the other child is null, then it is already root of the main path (and it is a backlink)
				// 3) switch if the other child is outside of the main path, and this way it would be closer to the main path
				{ // switch
					log.trace("switching " + oth.toString() + ((oth.parent != null) ? (" from " + oth.parent.toString()) : "") + " to " + el.toString());
					if(oth.parent != null)
					{
		...
		log.info("[node_name ( distance_from_root : parent_or_dash_if_root / number_of_children_or_dot_if_none +number_of_other_children)]" + "/ path_length_from_node)]");
